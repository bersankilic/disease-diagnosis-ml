...
def predict_disease(symptoms_input, model_name='Random Forest'):
    ...
    try:
        ...
        if model_name in models:
            ...
            # Numpy veri tiplerini Python standart tiplerine dönüştür
            ...
            if hasattr(model, 'predict_proba'):
                ...
                # Olasılıkları da Python float'a dönüştür
                ...
                disease_probs.sort(key=lambda x: x[1], reverse=True)                # Türkçe çeviri için hastalık isimlerini çevir
                turkish_probs = [(get_turkish_disease_name(disease), float(prob)) for disease, prob in disease_probs[:5]]

                # Detaylı hastalık bilgisini ekle                # Detaylı hastalık bilgisini ekle
                ...
                return {
                    ...
                    'prediction_english': str(prediction),  # Orijinal İngilizce ismi saklayalım
...
# Hastalık açıklamaları ve önerileri
disease_info = {
    'Mantar Enfeksiyonu': {
